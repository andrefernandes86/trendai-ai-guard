...
# Pre-flight: remove any orphaned log groups from a previous deployment.
# CloudFormation's EarlyValidation hook will reject the changeset if a log
# group with the same name already exists (even after the old stack was deleted).
ORPHAN_LOG_GROUPS=$(aws logs describe-log-groups --region "$REGION" \
...
if [[ -n "$ORPHAN_LOG_GROUPS" ]]; then
    warn "Orphaned log groups found from a previous deployment - removing before deploy:"
...
#!/usr/bin/env bash
# Generated by install.sh — safe to re-run after uninstall or on a new machine.
# Recreates the Lambda deploy bucket if missing, rebuilds and uploads the
# Lambda package, then deploys (or updates) the CloudFormation stack.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SCRIPT_EOF

# Embed the configuration values captured at install time
cat >> "$DEPLOY_SCRIPT" <<EOF
STACK_NAME="${STACK_NAME}"
REGION="${REGION}"
ACCOUNT_ID="${ACCOUNT_AT_INSTALL}"
DEPLOY_BUCKET="\${STACK_NAME}-deploy-\${ACCOUNT_ID}"
LAMBDA_S3_KEY="lambda/package.zip"
API_KEY="${API_KEY}"
ENDPOINT="${ENDPOINT}"
